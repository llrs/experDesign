...
-   [*OSAT*](https://bioconductor.org/packages/OSAT/), at Bioconductor,
    ...
    rows than the input provided because they might have NA value.
...
-   Recently, [*Omixer*](https://bioconductor.org/packages/Omixer/), a
    ...
    Omixer with other tools using the same settings.
...
![Flow chart to decide which functions are needed](flow_chart.png)
...
![Example of distributions generated with
***experDesign***.](paper_files/figure-markdown_strict/experDesign-1.png)

On Figure 2 one can see that the distribution index generated by
experDesign uses both numeric and categorical variables keeping also the
samples with missing (NA) information.
...
We are grateful to Joe Moore for English-language assistance and we
would like to thank reviewers and the editor for taking the time and
effort necessary to review the manuscript.
