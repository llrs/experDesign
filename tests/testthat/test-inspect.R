...
test_that("inspect works with replicates", {
  ...
  columns <- c("Sex", "Age", "Smoke")
  index <- replicates(
    pheno = survey[-nas, columns], size_subset = 70, controls = 5,
    iterations = 10
  )
...
test_that("inspect with translate_index", {
  ...
  nas <- c(137, 70)
  columns <- c("Sex", "Age", "Smoke")
  index1 <- replicates(survey[-nas, columns],
    size_subset = 50,
...
test_that("Warning on duplidate names", {
  data(survey, package = "MASS")
  columns <- c("Sex", "Age", "Smoke")
  expect_warning(index <- design(
    pheno = survey[, columns], size_subset = 70,
    iterations = 10
  ))
  batches <- inspect(index, survey[, columns])

  expect_warning(index2 <- design(
    pheno = batches, size_subset = 70,
    iterations = 10
  ))
