...
We can see that the sex is equally distributed and that the mean age ratio is 
...
is mostly similar to the original points.

# Unbalanced setting

In this case the data was mostly balanced (check it out in the `orig` object) 
...
```
We can see that in this simple case where a single variable has all the other cases we approximately reached the same entropy levels. We can avoid batch factor when the data
