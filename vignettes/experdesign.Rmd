...
If a data.frame has a column with name "batch" we can use the function distribution to see if it is randomlly distriubted.

```{r}
evaluate_category(index, survey[, c("Sex", "Smoke", "W.Hnd")])
evaluate_numbers(index, survey)
```

The numbers provided are just an indicator of how far are they from a perfect distance.
