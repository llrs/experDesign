...
Randomization and replicates can be utilized to prevent batch effects to confound the analysis.
...
***experDesign***, the package described on this paper, first evaluates the distribution of samples in batches and then on a spatial grid.
...
The distribution of samples that more closely match the original distribution is returned to the user.

This approach is similar to the *anticlust* method used for maximum variance, where the objective function is the sum of the squared errors between cluster centers and individual data points.
...
replicates_index <- replicates(survey[, VoI], size_subset = size_subset, controls = 5, iterations = iterations)
spatial_replicate_index <- spatial(replicates_index, survey[, VoI])
inspect(spatial_replicate_index, survey[, VoI])
```
