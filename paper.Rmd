...
```{r panel1, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figure A: distribution of  samples according to *OSAT*, Figure B according to *anticlust* on the variables used (1: Smoke, 2: Age, 3: Sex)."}
osat_plot <- osat_smoke / (osat_age + osat_sex)
osat_plot /anticlust + plot_layout(guides = 'collect') +
  plot_annotation(tag_levels = list(c("A.1", "A.2", "A.3", "B.2")),
                  title = "Existing solutions",
                  subtitle = "OSAT and anticlust solutions") &
  theme(plot.tag.position = c(0, 1),
        plot.tag = element_text(size = 8, hjust = 0, vjust = 0))
```

On Figure 1, we can see that the distributions is fairly similar between these two packages.
...
```{r experDesign, echo=FALSE, fig.cap="Sample, distributions with ***experDesign***.", warning=FALSE, message=FALSE}
library("experDesign")
