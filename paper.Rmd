...
There are several techniques to identify and take into account batch effects when analyzing an experiment already measured [@leek2010]. However, if the source of them are not taken into account before measuring they can introduce a batch effect that could be impossible to remove.
Therefore, it would be better if the batch effects were avoided before carrying out the experiment.
Tacking into account how the process from the design to the measurement can introduce variability will help avoiding batch effects.

To prevent the apparition of batch effect after the design of the experiment there are two options: randomization and replication.
This techniques can prevent the batch effect to be able to compare between or within the desired variable.
...
However, samples can be properly randomized to minimize the batch effect by looking how the variables distribute on each batch, this is known as randomized block experimental design or stratified random sampling experimental design.
...
Technical replicates help estimate the variation of the measurement method or the process.
...
For completeness a description of the usage of the different software
...
OSAT provides some template for some setups:

```{r osat}
library("OSAT")
gs <- setup.sample(survey, optimal = VoI)
gc <- setup.container(IlluminaBeadChip96Plate, n_batch, batch = 'plates')
...
```{r experDesign}
library("experDesign")
experDesign_index <- design(survey[, VoI], size_subset = 96,
