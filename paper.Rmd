...
**Blocking** is a method that groups samples that are equal according to one or more variables, allowing the estimation of the differences between each batch by comparing measurements within blocks.
...
Batches might be necessary for technical reasons, for example, the device cannot measure more than a number of samples at the same time.
...
By taking into account the de differences between the original design and the state before measurement is done, confounding effects could be minimized.

To prevent the batch effect fom confounding the analysis after the initial design of the experiment, there are two options: randomization and replication.
Randomization, consists on shuffling the samples to mix samples with different attributes, can help reduce variations across groups, while replication helps estimate the variation of the measurements or samples, thus increasing the precision of the estimates of the true value on the analysis.
...
There are some tools to minimize batch effects on the R language in multiple fields, and particularly for biological research [@rcoreteam2014].
...
Current solutions to stratify samples to reduce and control batch effect do not work for all cases.
They are either specialized to particular type of data, they omit some conditions that are often met or they only work on a specific subset of conditions.
The new package ***experDesign*** works on all kind of data types and does not require a spatial distribution making it suitable for all kind of experiments.
This package is intended for people looking for a quick and easy solution providing reasonable suggestions on how to distribute the samples for their analysis.
...
First, we start with some real data from a survey:
...
OSAT provides some template for plates that hold 2, 4, 8 Illumina BeadChip chips and have 24, 48, 96 wells and works for numeric and categorical variables:
...
OSAT returns one row less that the input provided.
This row is deleted because it has an NA value on the Sex variable.
...
On each iteration if the random distribution of samples' statitics for each batch has fewer differences with the original distribution than the last stored sample distribution then it replaces it as the best sample distribution.
...
Samples are distributed on each position as with the `design` function, only returning to the user the distribution with less differences between the statistics previously mentioned with the original distribution of all the iterations tried.
